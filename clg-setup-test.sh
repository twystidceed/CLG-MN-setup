...
echo "Do you want me to setup a swap file for you? Y if you are unsure [y/n]"
  read DOSETUP
if [[ $DOSETUP =~ "y" ]] ; then
...
rm -rf collegicoin-v3.0.0-linux64.zip
 ...
 if [ ! -f ~/.collegicoin/collegicoin.conf ]; then 
...
if [ -z $genkey ]; then
    cat <<EOF > ~/.collegicoin/collegicoin.conf
rpcuser=$rpcuser
rpcpassword=$rpcpassword
EOF

    sudo chmod 755 -R ~/.collegicoin/collegicoin.conf
...
${YELLOW}cat ~/.collegicoin/collegicoin.conf${NC}
Here is your collegicoin.conf generated by this script:
-------------------------------------------------${YELLOW}"
cat ~/.collegicoin/collegicoin.conf
echo -e "${NC}-------------------------------------------------
